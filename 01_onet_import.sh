...
mysql --local-infile=1 -Donet -e "LOAD DATA LOCAL INFILE '/tmp/translations/content_model_reference_nb.tsv' REPLACE INTO TABLE content_model_reference_nb FIELDS TERMINATED BY '\t' ENCLOSED BY '\"' IGNORE 1 LINES"
echo "import of content_model_reference_nb"

mysql -Donet -e "TRUNCATE dwa_reference_nb"
mysql --local-infile=1 -Donet -e "LOAD DATA LOCAL INFILE '/tmp/translations/dwa_reference_nb.tsv' REPLACE INTO TABLE dwa_reference_nb FIELDS TERMINATED BY '\t' ENCLOSED BY '\"' IGNORE 1 LINES"
echo "import of dwa_reference_nb"

mysql -Donet -e "TRUNCATE iwa_reference_nb"
mysql --local-infile=1 -Donet -e "LOAD DATA LOCAL INFILE '/tmp/translations/iwa_reference_nb.tsv' REPLACE INTO TABLE iwa_reference_nb FIELDS TERMINATED BY '\t' ENCLOSED BY '\"' IGNORE 1 LINES"
echo "import of iwa_reference_nb"

mysql -Donet -e "TRUNCATE occupation_data_nb"
mysql --local-infile=1 -Donet -e "LOAD DATA LOCAL INFILE '/tmp/translations/occupation_data_nb.tsv' REPLACE INTO TABLE occupation_data_nb FIELDS TERMINATED BY '\t' ENCLOSED BY '\"' IGNORE 1 LINES"
